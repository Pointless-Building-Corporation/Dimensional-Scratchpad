...
public class ServerCommonEvents {
    ...
    public static void onAttachCapabilities(AttachCapabilitiesEvent<Entity> event) {
        if (event.getObject() instanceof Player) {
            event.addCapability(new ResourceLocation(DimensionalScratchpad.MODID, "scratchpad_state"), new ScratchpadState());
        }
    }

    @SubscribeEvent
    public static void OnPlayerClone(PlayerEvent.Clone event) {
        event.getOriginal().getCapability(ScratchpadState.SCRATCHPAD_STATE).ifPresent(oldCap -> {
